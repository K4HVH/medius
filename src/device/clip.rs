...
impl ClipHandle {
    ...
    /// Make the clip's motion wait to ride a native report under [`set_movement_riding`](crate::Device::set_movement_riding) (`false` = the box's own clock, the default); only its wheel while rendering is on with a profile armed. Changeable mid-playback. Fire-and-forget.
