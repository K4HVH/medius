...
//! [`Device::logs`](crate::Device::logs) hands callers a clone of the receiver. The bound is the safety
//! property: a consumer that never drains must not let the reader allocate without limit.
//!
...
impl Device {
    ...
    ///
    /// A clone of the bounded channel's receiver; callers may each hold one (flume is MPMC). On
    /// overflow the **oldest** buffered line is dropped, so a slow consumer can never stall the reader.
    /// Lines arrive in the order the box sent them.
