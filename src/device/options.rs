...
impl Device {
    /// `OPTION(IMPERFECT)`: opt into cloning a device the box cannot clone exactly, or back to
    ...
    ///
    /// A toggle that changes the patch set the clone serves presents the clone again, which releases
    /// the session like a replug of the device; the crate re-sends what it holds once the new clone is
    /// up, and [`ClipHandle::lost`](crate::ClipHandle::lost) reports a clip it dropped.
    ...
    /// across one command interval. Above 100 overlaps, and is allowed. A loop at the native report rate
    /// keeps each command whole, on a report of its own.
    ///
