...
impl Device {
    ...
    /// Best-effort reconnect: rescan by VID/PID, reopen, re-apply held state, bump the counter. A
    /// device chip that restarted meanwhile gets the held state once its clone is up.
