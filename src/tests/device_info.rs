...
fn decode_stats_exact_bytes_with_saturation() {
    // Same vector as the firmware packer test. The last eight bytes are the two link drop
    // counters, which are full width: 0xDEADBEEF would have saturated had they been narrowed.
    let p = [
        5u8, 0x04, 0x03, 0x02, 0x01, 0xFF, 0xFF, 0x0A, 0x00, 0xFF, 0x02, 0xFF, 0xFF, 0x07, 0x00,
        0x09, 0x00, 0xEF, 0xBE, 0xAD, 0xDE, 0x04, 0x03, 0x02, 0x01,
    ];
...
fn truncated_payloads_decode_to_none() {
    ...
    assert!(parse_resp(&[5, 0, 0, 0]).is_none()); // STATS needs 25
