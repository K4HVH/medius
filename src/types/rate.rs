...
impl Rate {
    /// The native report rate in Hz, or `None` when there is no continuous cadence.
    ...
    /// Decode a `RESP(RATE)` payload (§4.5): `[what][native_period_us u16][poll_period_us u16][flags u8]`.
