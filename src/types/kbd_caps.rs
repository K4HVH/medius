...
/// Cloned keyboard's capabilities; the keyboard half of [`Caps`](crate::Caps).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct KbdCaps {
    /// Keycode-array slots the report carries, or `0xFF` when the keyboard uses an NKRO bitmap.
    pub n_keys: u8,
    /// Keys are an NKRO bitmap (no rollover limit), not a fixed keycode array.
    pub nkro: bool,
    /// A Consumer (media-key) collection is present; media injection and catch work.
